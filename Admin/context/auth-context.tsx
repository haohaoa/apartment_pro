...
type AuthContextType = {
    listUsers: User[];
    user: User | null;
    UserDeatail: User | null;
    getByIdUser: (id: string) => Promise<boolean>;
    updateUser: (id: string, data: User) => Promise<boolean>;
    getDelete: (id: string) => Promise<boolean>;
...
export function AuthProvider({ children }: { children: ReactNode }) {
    ...
    const [UserDeatail, setUserDetail] = useState<User | null>(null)
    ...
    useEffect(() => {
        ...
        if (storedUser) {
            ...
            if (parsedUser.role != "admin") logout();
    ...
    const getalluser = async (): Promise<boolean> => {
        try {
            const res = await AuthAPI.getAll();
            const dataUser: User[] = res.data.users;
    ...
    const getDelete = async (id: string): Promise<boolean> => {
        try {
    ...
    const updateUser = async (id: string, data: User): Promise<boolean> => {
        try {
    ...
    const getByIdUser = async (id: string): Promise<boolean> => {
        try {
    ...
    return (
        <AuthContext.Provider value={{ user, listUsers, UserDeatail, getByIdUser, updateUser, getDelete, getalluser, setUsers, verifyOTP, resendOTP, login, register, authGoogle, logout }}>
            {children}
